...
s-maxage 与 max-age 指令相同，但是不同点是 s-maxage 指令只适用于供多位用户使用的公共缓存服务器（一般为代理），也可以说对于向同一用户重复返回响应的服务器来说，这个指令没有任何作用。同时，使用此字段后，expires 和 max-age 字段无效。
...
must-revalidate 表示当缓存在有效期内，会直接使用浏览器缓存，不会发起服务端请求。当缓存过期了，客户端会再次发起请求（带有 If-Modified-Since/If-None-Match 请求头），如果服务端返回的状态码是 304，则客户端会根据该 304 响应所包含的一些响应头（Date、Last-Modified、Cache-Control 等）重新计算出这条缓存的过期时间。
...
301 表示永久重定向，当服务端返回 301 状态码，并在 Response Header 中写入 `Location`（重定向地址），浏览器会自动根据 `Location` 的值进行跳转。值得注意的是，当服务端返回 301 状态码后，浏览器会将路径的重定向关系缓存到本地，当下次访问的时候会直接读取本地缓存，然后进行跳转。即使服务端已经修改了重定向地址也不会使用新的重定向地址，除非清除本地的缓存。

#### 302

302 表示临时重定向，当服务端返回 302 状态码，并在 Response Header 中写入 `Location`（重定向地址），浏览器会自动根据 `Location` 的值进行跳转。与 301 不同的是，浏览器不会返回 302 的结果，每次访问页面都会请求服务端。当服务端更新后，不需要清除缓存就能匹配。
...
2. [HTTP 灵魂之问，巩固你的 HTTP 知识体系](https://juejin.cn/post/6844904100035821575)
3. [可能是全网最全的 http 面试答案](https://juejin.cn/post/6844903865410650126)
